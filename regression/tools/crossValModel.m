function [Vm, cBeta, cR, subIdx, cRidge, cLabels] =  crossValModel(fullR, Vc, cLabels, regIdx, regLabels, folds, dataIdx)
% function to compute cross-validated R^2

rng(1) % for reproducibility

if nargin < 7
    dataIdx = true(folds, size(Vc,2));
    randIdx = randperm(size(Vc,2)); %generate randum number index
    foldCnt = floor(size(Vc,2) / folds);
    for iFolds = 1:folds
        dataIdx(iFolds, randIdx(((iFolds - 1)*foldCnt) + (1:foldCnt))) = false; %index for training data
    end
end
...
for iFolds = 1:folds
    ...
    if folds > 1
        ...
        if iFolds == 1
            [cRidge, cBeta{iFolds}] = ridgeMML(Vc(:,dataIdx(iFolds, :))', cR(dataIdx(iFolds, :),:), true); %get beta weights and ridge penalty for task only model
        else
            [~, cBeta{iFolds}] = ridgeMML(Vc(:,dataIdx(iFolds, :))', cR(dataIdx(iFolds, :),:), true, cRidge); %get beta weights for task only model. ridge value should be the same as in the first run.
        end
        
        Vm(:,~dataIdx(iFolds, :)) = ((cR(~dataIdx(iFolds, :),:) - mean(cR(~dataIdx(iFolds, :),:), 1)) * cBeta{iFolds})' + mean(Vc(:,dataIdx(iFolds, :)), 2);% (X - mean(X, 1)) * betas %predict remaining data
        ...
        Vm = ((cR  - mean(cR, 1)) * cBeta{iFolds})' + mean(Vc, 2); %predict remaining data
