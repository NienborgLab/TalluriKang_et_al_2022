function [mean_stim_rates, blockwise_mean_stim_rates] = stim_rates_le(Vc, D)
% stim_rates = nan(size(Vc, 1), size(D.blocks_clean, 1)*4);
...
% %         keyboard
% %     end
%     stim_rates(:, k) = sum(Vc(:, stim_start: stim_start+30), 2)/0.5;
%     k = k + 1;
%     stim_rates(:, k) = sum(Vc(:, stim_start+31: stim_start+60), 2)/0.5;
%     k = k + 1;
%     stim_rates(:, k) = sum(Vc(:, stim_start+61: stim_start+90), 2)/0.5;
%     k = k + 1;
%     stim_rates(:, k) = sum(Vc(:, stim_start+91: stim_start+120), 2)/0.5;
%     k = k + 1;
% end
